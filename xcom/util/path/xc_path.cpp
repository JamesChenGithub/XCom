...
std::vector<TXCPath> TXCPath::childs() const{
    std::vector<TXCPath> list;
#if defined(_WIN32)
...
std::wstring TXCPath::wstr(path_type type) const {
...
void TXCPath::set(const std::wstring &wstring, path_type type) {
    std::string string;
    if (!wstring.empty()) {
