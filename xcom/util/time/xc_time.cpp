...
uint64_t xc_clock_app_monotonic() {
    struct timespec ts;
...
uint64_t xc_get_tick_count() {//todoyy
    struct timespec ts;
...
int64_t xc_get_tick_span(uint64_t _old_tick) {
    uint64_t cur_tick = xc_get_tick_count();
